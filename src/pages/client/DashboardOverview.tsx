...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    getDashboardStats().then(({ stats: s, invoices, payments }) => {
      setStats(s);
  ...
  const widgets = [
    { icon: Server, label: "Active Services", value: loading ? "—" : String(stats.services), trend: "+2 this month", bgGlow: "bg-primary/10", iconColor: "text-primary" },
    { icon: Globe, label: "Active Domains", value: loading ? "—" : String(stats.domains), trend: "All healthy", bgGlow: "bg-emerald-500/10", iconColor: "text-emerald-500" },
    { icon: FileText, label: "Pending Invoices", value: loading ? "—" : String(stats.pendingInvoices), trend: stats.pendingInvoices > 0 ? "Action needed" : "All clear", bgGlow: "bg-amber-500/10", iconColor: "text-amber-500" },
    { icon: CreditCard, label: "Total Spent", value: loading ? "—" : `KSh ${stats.totalSpent.toLocaleString()}`, trend: "Lifetime", bgGlow: "bg-accent/10", iconColor: "text-accent" },
  ];

  const getStatusConfig = (status: string) => {
    switch (status) {
      case "paid": return { class: "bg-emerald-500/10 text-emerald-600 border-emerald-200", dot: "bg-emerald-500" };
      case "unpaid": return { class: "bg-amber-500/10 text-amber-600 border-amber-200", dot: "bg-amber-500" };
      default: return { class: "bg-red-500/10 text-red-600 border-red-200", dot: "bg-red-500" };
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {widgets.map((w, i) => (
          <motion.div key={w.label} custom={i} initial="hidden" animate="visible" variants={fadeUp}
              ...
              return (
                <div key={inv.id} className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-muted/30 transition-colors">
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold border ${sc.class}`}>{inv.status}</span>
                    <span className="text-xs text-muted-foreground hidden sm:block">{new Date(inv.issued_at).toLocaleDateString()}</span>
                  </div>
                </div>
              );
            })}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }}
          ...
          <div className="rounded-2xl bg-card border border-border/50 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/client/dashboard/payments" className="text-xs text-primary hover:text-primary/80 font-medium">View All</Link>
            </div>
                ...
                {recentPayments.map((p: any) => (
                  <div key={p.id} className="flex items-center justify-between py-2">
                    ...
                    <div className="text-right">
                      ...
                      <div className={`text-[10px] font-semibold ${p.status === "completed" ? "text-emerald-600" : "text-amber-600"}`}>{p.status}</div>
                    </div>
                  </div>
                ))}
              </div>
