...
import { checkHostingStatus, getPanelStats, getSsoUrl, type PanelStats } from "@/lib/api";
...
export default function ClientCpanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const status = await checkHostingStatus();
      if (!status.has_hosting) {
        setHasHosting(false);
        setLoading(false);
        return;
      }
      ...
      setHasHosting(true);
      setDomain(status.domain || "No domain set");

      const panelStats = await getPanelStats();
      if (panelStats) setStats(panelStats);
    } catch (err) {
  ...
  const handleOpenPanel = async () => {
    setSsoLoading(true);
    try {
      const sso = await getSsoUrl();
      if (sso?.url) {
        window.open(sso.url, "_blank");
      } else {
        console.error("SSO: No URL returned");
      }
    } catch (err) {
          ...
          <div className="relative">
            ...
            <Link to="/client/dashboard/hosting">
              <Button size="lg" className="gradient-primary text-primary-foreground border-0 mt-6 rounded-xl px-8 shadow-lg hover:shadow-xl transition-shadow">
                <ShoppingCart className="w-5 h-5 mr-2" /> Browse Hosting Plans
              </Button>
            </Link>
          </div>
        </motion.div>
  ...
  const statCards = [
    { icon: HardDrive, label: "Disk Space", value: stats ? `${stats.disk.used_mb} MB` : "—", max: stats ? `${stats.disk.limit_mb === -1 ? "∞" : stats.disk.limit_mb + " MB"}` : "—", percent: stats?.disk.percent ?? 0, gradient: "from-blue-500 to-cyan-500", glow: "bg-blue-500/10" },
    { icon: Gauge, label: "Bandwidth", value: stats ? `${stats.bandwidth.used_mb} MB` : "—", max: stats ? `${stats.bandwidth.limit_mb === -1 ? "∞" : stats.bandwidth.limit_mb + " MB"}` : "—", percent: stats?.bandwidth.percent ?? 0, gradient: "from-violet-500 to-purple-500", glow: "bg-violet-500/10" },
    { icon: Database, label: "Databases", value: stats ? `${stats.databases.count}` : "—", max: stats ? `${stats.databases.limit === -1 ? "∞" : stats.databases.limit}` : "—", percent: stats ? (stats.databases.limit > 0 ? (stats.databases.count / stats.databases.limit) * 100 : 0) : 0, gradient: "from-emerald-500 to-teal-500", glow: "bg-emerald-500/10" },
    { icon: Mail, label: "Email Accounts", value: stats ? `${stats.email.count}` : "—", max: stats ? `${stats.email.limit === -1 ? "∞" : stats.email.limit}` : "—", percent: stats ? (stats.email.limit > 0 ? (stats.email.count / stats.email.limit) * 100 : 0) : 0, gradient: "from-amber-500 to-orange-500", glow: "bg-amber-500/10" },
  ];
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 flex items-center gap-2">
            ...
            {stats?.expires_at && <span className="text-xs ml-2 px-2 py-0.5 rounded-full bg-muted">Expires {new Date(stats.expires_at).toLocaleDateString()}</span>}
          </p>
        </div>
        <Button size="lg" className="bg-gradient-to-r from-accent to-accent/90 text-accent-foreground border-0 rounded-xl shadow-lg hover:shadow-xl transition-all"
          onClick={handleOpenPanel} disabled={ssoLoading}>
          ...
          <motion.div key={stat.label} custom={i} initial="hidden" animate="visible" variants={fadeUp}
            ...
            <div className="relative">
              ...
              <div className="mt-4 w-full h-2 bg-muted/50 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(stat.percent, 100)}%` }}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <Button variant="ghost" size="sm" className="text-xs text-muted-foreground" onClick={handleRefresh} disabled={refreshing}>
            <RefreshCw className={`w-3.5 h-3.5 mr-1.5 ${refreshing ? "animate-spin" : ""}`} /> Refresh
          </Button>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {quickActions.map((action, i) => (
            <motion.button key={action.label} custom={i} initial="hidden" animate="visible" variants={fadeUp}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
        ...
        <Button variant="outline" size="sm" onClick={handleOpenPanel} className="rounded-lg shrink-0 hidden sm:flex">
          <ArrowUpRight className="w-4 h-4 mr-1" /> Launch
        </Button>
      </motion.div>
    </div>
  );
